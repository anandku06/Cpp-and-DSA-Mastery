...
// Intuition
// We can use a stack to keep track of the elements in nums2. We will iterate through nums2 and for each element, we will pop the elements from the stack until we find an element that is greater than the current element. The popped elements will have the current element as their next greater element. We will also store the next greater element for each popped element in a hash map. Finally, we will iterate through nums1 and for each element, we will check if it has a next greater element in the hash map. If it does, we will add it to the answer; otherwise, we will add -1 to the answer.

// approach: Using a Hash Map
// Intuition
// We can use a hash map to store the next greater element for each element in nums2. We will iterate through nums2 and for each element, we will check if it has a next greater element in the hash map. If it does, we will add it to the answer; otherwise, we will add -1 to the answer. This approach is less efficient than using a stack, as it takes O(nums1.length * nums2.length) time to find the next greater element for each element in nums1, while using a stack takes O(nums1.length + nums2.length) time.
