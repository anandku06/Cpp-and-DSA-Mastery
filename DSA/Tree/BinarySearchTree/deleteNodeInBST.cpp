...
// 1. The node to be deleted has no children (it's a leaf node).
    ...
    TreeNode *deleteNode(TreeNode *root, int key)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                root->val = minNode->val;                            // Replace the value of the node to be deleted with the minimum value
                root->right = deleteNode(root->right, minNode->val); // Delete the minimum node from the right subtree
            }
        }

        return root; // Return the (possibly updated) root of the BST
    }
};